...
object LU extends Verification {
  ...
  override def run: Unit = {
    ...
    println(s"\tComparing L : ${Verification.arrayCompare2D(jluSquare.getL.getArray, mluSquare.getL().values)}")
    println(s"\tComparing U : ${Verification.arrayCompare2D(jluSquare.getU.getArray, mluSquare.getU().values)}")
...
//    println(s"\tComparing U : ${Verification.arrayCompare2D(jluWide.getU.getArray, mluWide.getU().values)}")
    ...
    println(s"\tComparing L : ${Verification.arrayCompare2D(jluTall.getL.getArray, mluTall.getL().values)}")
    println(s"\tComparing U : ${Verification.arrayCompare2D(jluTall.getU.getArray, mluTall.getU().values)}")
  }
}
