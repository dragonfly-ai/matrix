...
object Eigen extends Verification {
  ...
  override def run: Unit = {
    ...
    val med: matrix.decomposition.Eigen = matrix.decomposition.Eigen(mm)
    ...
    println(s"\tComparing V : ${Verification.arrayCompare2D(jed.getV.getArray, med.getV().values)}")
    println(s"\tComparing D : ${Verification.arrayCompare2D(jed.getD.getArray, med.getD().values)}")
